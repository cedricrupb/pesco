...
def _index_elements(obj, index):
    try:
        ...
        rindex = {i: o for i, o in enumerate(obj)}
        return [rindex[ix] for ix in index]
...
def stratified_split(dataset, ratio=0.3):
    ...
    instance_idx = {k: i for i, k in enumerate(dataset.instance_index)}
    for i, instance in enumerate(test_dataset.instance_index):
        test_labels = test_dataset.labels[i]
        org_labels  = dataset.labels[instance_idx[instance]]
        if (test_labels - org_labels).sum() != 0:
            print(instance, test_labels, org_labels)
...
def _has_solved(entry):
    ...
    return entry["status"].startswith(str(entry["verdict"]).lower())
